...
namespace QuantConnect.DataLibrary.Tests
{
    ...
    public class CBOEDataAlgorithm : QCAlgorithm
    {
        ...
        public override void Initialize()
        {
            ...
            _equitySymbol = AddEquity("SPY", Resolution.Hour).Symbol;
            _customDataSymbol = AddData<CBOE>("VIX").Symbol;
        }
        ...
        {
            var data = slice.Get<CBOE>();
            if (!data.IsNullOrEmpty())
            {
                // based on the custom data property we will buy or sell the crude ETF equity
                if (data[_customDataSymbol].High >= 10m)
                {
                    Liquidate(_equitySymbol);
                }
                else if (Portfolio.TryGetValue(_equitySymbol, out var holdings) && !holdings.Invested)
                {
